...
class Q {
  ...
  run(t, e, i) {
    const s = this, r = s.options, n = function(l) {
      ...
      for (let l = 0; l < Q.timers.length; l++)
        Q.timers[l]() || Q.timers.splice(l--, 1);
      Q.timers.length && o(a);
    };
    t === e && !this.elem["forceAnimate:" + this.prop] ? (delete r.curAnim[this.prop], r.complete && Object.keys(r.curAnim).length === 0 && r.complete.call(this.elem)) : (this.startTime = +/* @__PURE__ */ new Date(), this.start = t, this.end = e, this.unit = i, this.now = this.start, this.pos = 0, n.elem = this.elem, n.prop = this.prop, n() && Q.timers.push(n) === 1 && o(a));
  }
  /**
   ...
   */
  fillSetter() {
    Q.prototype.strokeSetter.apply(this, arguments);
  }
  /**
   * Handle animation of the color attributes directly.
   *
...
Q.timers = [];
const { getStyle: Yr, isArray: Vr, isNumber: Ur, isObject: Js, merge: Qs, objectEach: Kr, pick: qr } = G;
function Zr(h, t) {
  t.renderer.globalAnimation = qr(h, t.options.chart.animation, !0);
}
function _r(h) {
  return Js(h) ? Qs({ duration: 500, defer: 0 }, h) : { duration: h ? 500 : 0, defer: 0 };
}
function Jr(h, t, e) {
  let i, s = "", r, n, o;
  Js(e) || (o = arguments, e = {
    ...
    easing: o[3],
    complete: o[4]
  }), Ur(e.duration) || (e.duration = 400), e.easing = typeof e.easing == "function" ? e.easing : Math[e.easing] || Math.easeInOutSine, e.curAnim = Qs(t), Kr(t, function(a, l) {
    tr(h, l), n = new Q(h, e, l), r = void 0, l === "d" && Vr(t.d) ? (n.paths = n.initPath(h, h.pathArray, t.d), n.toD = t.d, i = 0, r = 1) : h.attr ? i = h.attr(l) : (i = parseFloat(Yr(h, l)) || 0, l !== "opacity" && (s = "px")), r || (r = a), typeof r == "string" && r.match("px") && (r = r.replace(/px/g, "")), n.run(i, r, s);
  });
}
function tr(h, t) {
  let e = Q.timers.length;
  for (; e--; )
    Q.timers[e].elem === h && (!t || t === Q.timers[e].prop) && (Q.timers[e].stopped = !0);
}
const qt = {
  animate: Jr,
  animObject: _r,
...
const { animObject: Dn } = qt, { xAxis: qi, yAxis: $n } = Qr, { defaultOptions: Mi } = St, { registerEventOptions: In } = Ei, { deg2rad: Bn } = I, { arrayMax: Zi, arrayMin: Nn, clamp: je, correctFloat: J, defined: R, destroyObjectProperties: Rn, erase: _i, error: He, extend: ve, fireEvent: W, getClosestDistance: Ji, insertItem: zn, isArray: Qi, isNumber: C, isString: ts, merge: Se, normalizeTickInterval: Wn, objectEach: re, pick: L, relativeLength: ne, removeEvent: Fn, splat: jn, syncTimeout: Hn } = G, es = (h, t) => Wn(t, void 0, void 0, L(
...
class Ut {
  ...
  translate(t, e, i, s, r, n) {
    ...
    return p || (p = o.transA), i && (f *= -1, d = o.len), o.reversed && (f *= -1, d -= f * (o.sector || o.len)), e ? (t = t * f + d, t -= l, u = t / p + a, c && (u = o.lin2val(u))) : (c && (t = o.val2lin(t)), u = f * (t - a) * p + d + f * l + (C(n) ? p * n : 0), o.isRadial || (u = J(u))), u;
  }
  /**
   ...
   */
  nameToX(t) {
    const e = Qi(this.options.categories), i = e ? this.categories : this.names;
    let s = t.options.x, r;
...
const Un = Ci, { SVG_NS: rs, win: Kn } = I, { attr: qn, createElement: Zn, css: _n, error: ns, isFunction: Jn, isString: os, objectEach: as, splat: Qn } = G, { trustedTypes: Ye } = Kn, Ae = Ye && Jn(Ye.createPolicy) && Ye.createPolicy("highcharts", {
  createHTML: (h) => h
}), to = Ae ? Ae.createHTML("") : "";
class X {
  /* *
  ...
  addToDOM(t) {
    function e(i, s) {
      let r;
      return Qn(i).forEach(function(n) {
  ...
  */
  ...
  getRotatedBox(t, e) {
    const { x: i, y: s, width: r, height: n } = t, { alignValue: o, translateY: a, rotationOriginX: l = 0, rotationOriginY: c = 0 } = this, f = qe(o), d = Number(this.element.getAttribute("y") || 0) - (a ? 0 : s), p = e * fs, u = (e - 90) * fs, g = Math.cos(p), x = Math.sin(p), m = r * g, b = r * x, y = Math.cos(u), v = Math.sin(u), [[w, S], [k, M]] = [
      ...
      wt - wt * g,
      wt * x
    ]), A = i + f * (r - m) + w + M, T = s + d - f * b - S + k, O = A + d * y, B = O + m, $ = B - n * y, N = $ - m, P = T + d * v, D = P + b, z = D - n * v, q = z - b, tt = Math.min(O, B, $, N), _t = Math.min(P, D, z, q), Oe = Math.max(O, B, $, N) - tt, Jt = Math.max(P, D, z, q) - _t;
    return {
      x: tt,
      y: _t,
      width: Oe,
...
class et {
   ...
   */
  afterAnimate() {
    this.setClip(), Je(this.chart.sharedClips, (t, e, i) => {
  ...
  destroy(t) {
    ...
    for (const l of e.zones)
      No(l, void 0, !0);
    G.clearTimeout(e.animationTimeout), Je(e, function(l, c) {
      l instanceof H && !l.survive && (n = s && c === "group" ? "hide" : "destroy", l[n]());
    }), i.hoverSeries === e && (i.hoverSeries = void 0), ws(i.series, e), i.orderItems("series"), Je(e, function(l, c) {
...
const Zo = {
  ...
  triangle: Ko,
  "triangle-down": qo
}, { doc: Qe, SVG_NS: _o, win: Ds } = I, { attr: ti, extend: Jo, fireEvent: Qo, isString: ta, objectEach: ea, pick: ia } = G, ei = (h, t) => h.substring(0, t) + "…";
class sa {
  constructor(t) {
    const e = t.styles;
  ...
  buildSVG() {
    ...
    if (l !== t.textCache) {
      ...
      if (!r && !this.ellipsis && !this.width && !t.textPath && (s.indexOf(" ") === -1 || this.noWrap && !a.test(s)))
        e.appendChild(Qe.createTextNode(this.unescapeEntities(s)));
      else if (s !== "") {
        o && o.appendChild(e);
        const c = new X(s);
    ...
    const r = (o, a) => {
      ...
      if (f) {
        ...
        m.forEach((b) => {
          ...
          a.insertBefore(Qe.createTextNode(b), o);
          const y = Qe.createElementNS(_o, "tspan");
  ...
  modifyTree(t) {
    const e = (i, s) => {
      ...
      a !== "#text" && a !== "a" && (i.tagName = "tspan"), Jo(i, { attributes: r, style: o }), n && n.filter((c) => c.tagName !== "#text").forEach(e);
    };
    t.forEach(e), Qo(this.svgElement, "afterModifyTree", { nodes: t });
  }
  /*
...
class xt {
  ...
  transform(t) {
    ...
    for (const u of e) {
      ...
      const { dataMin: _t, dataMax: Oe, min: Jt, max: wt } = dt(u.getExtremes(), tt || {}), Le = c.parse(b.min), Ee = c.parse(b.max), $i = _t ?? Le, Ii = Oe ?? Ee, Pe = q - z, Bi = u.categories ? 0 : Math.min(Pe, Ii - $i), De = $i - Bi * (ct(Le) ? 0 : b.minPadding), $e = Ii + Bi * (ct(Ee) ? 0 : b.maxPadding), Ni = u.allowZoomOutside || M === 1 || a !== "zoom" && M > 1, Qt = Math.min(Le ?? De, De, Ni ? Jt : De), te = Math.max(Ee ?? $e, $e, Ni ? wt : $e);
      (!u.isOrdinal || M !== 1 || r) && (z < Qt && (z = Qt, M >= 1 && (q = z + Pe)), q > te && (q = te, M >= 1 && (z = q - Pe)), (r || u.series.length && (z !== Jt || q !== wt) && z >= Qt && q <= te) && (n ? n[u.coll].push({
        axis: u,
        min: z,
        max: q
      }) : (u.isPanning = a !== "zoom", u.isPanning && (p = !0), u.setExtremes(r ? void 0 : z, r ? void 0 : q, !1, !1, { move: O, trigger: a, scale: M }), !r && (z > Qt || q < te) && a !== "mousewheel" && (d = !0)), f = !0), !this.hasCartesianSeries && !r && a !== "mousewheel" && (d = !0), i && (this[g ? "mouseDownX" : "mouseDownY"] = i[g ? "chartX" : "chartY"]));
    }
    return f && (n ? E(
      this,
      "selection",
